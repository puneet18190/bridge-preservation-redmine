...
class DealCategoriesController < ApplicationController
  ...
  before_filter :find_model_object, :except => [:new, :index, :create]
  before_filter :find_project_from_association, :except => [:new, :index, :create]
  before_filter :find_project_by_project_id, :only => [:new, :index, :create]
