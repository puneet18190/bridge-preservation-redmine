...
class DealsPipelineProcessor
  ...
  def closed_deals_for_status(status)
    ...
    ret = @scope.closed.joins("LEFT OUTER JOIN #{DealProcess.table_name} dp on dp.deal_id = deals.id AND NOT (#{first_condition.join(' AND ')})").
      joins("LEFT OUTER JOIN #{DealProcess.table_name} dp2 ON (deals.id = dp2.deal_id AND (dp.created_at < dp2.created_at OR dp.created_at = dp2.created_at AND dp.id < dp2.id)) AND NOT (#{second_condition.join(' AND ')})").
