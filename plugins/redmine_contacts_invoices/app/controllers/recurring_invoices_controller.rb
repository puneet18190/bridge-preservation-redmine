...
class RecurringInvoicesController < ApplicationController
  ...
  def new
    ...
    @recurring_invoice.copy_from(params[:copy_from]) if params[:copy_from]
    if params[:issues_ids]
      @recurring_invoice.lines_from_issues(params[:issues_ids], params[:line_grouping], :project => @project)
    elsif params[:time_entries_ids]
      @recurring_invoice.lines_from_time_entries(params[:time_entries_ids], params[:line_grouping], :project => @project)
    end
  ...
  def client_view
    @payments = @recurring_invoice.payments
    @comments = @recurring_invoice.comments.to_a
    @comments.reverse! if User.current.wants_comments_in_reverse_order?

    respond_to do |format|
      format.html do
        if RecurringInvoicesSettings.is_custom_template?(@project)
          @html_preview = true
          render :text => liquidize_invoice(@invoice), :layout => 'invoices.pdf'
        else
          send_data(invoice_to_pdf(@invoice), :type => 'application/pdf', :filename => "invoice-#{@recurring_invoice.number}.pdf", :disposition => 'inline')
        end
      end
      format.pdf  { send_data(invoice_to_pdf(@invoice), :type => 'application/pdf', :filename => "invoice-#{@recurring_invoice.number}.pdf", :disposition => 'inline') }
    end
