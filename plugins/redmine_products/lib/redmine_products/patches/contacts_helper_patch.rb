      ...
      module InstanceMethods
        ...
        def contact_tabs_with_orders(contact)
          ...
          if contact.orders.visible.count > 0
            tabs.push({:name => 'orders', :partial => 'contacts/related_orders', :label => l(:label_order_plural) + " (#{contact.orders.visible.count})" } )
          end
          if contact.products.visible.count > 0
            tabs.push({:name => 'products', :partial => 'contacts/related_products', :label => l(:label_product_plural) + " (#{contact.products.visible.count})" } )
          end
          tabs
        end
