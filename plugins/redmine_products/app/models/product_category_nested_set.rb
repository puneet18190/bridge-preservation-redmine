...
module ProductCategoryNestedSet
  ...
  def target_lft
    ...
    if siblings_rgt
      ...
      raise "ProductCategory id=#{id} with parent_id=#{parent_id}: parent missing or without 'lft' value" unless parent_lft
      parent_lft + 1
    else
      1
    end
  ...
  def reload_nested_set_values
    self.lft, self.rgt = ProductCategory.where(:id => id).pluck(:lft, :rgt).first
  end
